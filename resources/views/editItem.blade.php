...
<form action="/update-item/{{ $item->id }}" method = "POST">
  ...
  <div class="mb-3">
    ...
    <textarea type="text" class="form-control" id="exampleInputEmail1" cols="30" rows="10" aria-describedby="emailHelp" name = 'description' value = "{{ old('description', $item->description) }}"> </textarea>
    @error('description')
      <div class = "alert alert-danger"> {{$message}} </div>
    @enderror
  </div>
